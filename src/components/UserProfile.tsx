// Remove explicit React import since we're using automatic runtime
import { useState, useEffect } from 'react';
import { LogOut } from 'lucide-react';
import { signOut, getCurrentSession, onAuthStateChange } from '../lib/supabase/auth';
...
export default function UserProfile({ isAuthenticated, onLogin, onLogout }: UserProfileProps) {
  ...
  if (!isAuthenticated) {
    return (
      <button
        onClick={onLogin}
        className="px-6 py-2.5 bg-white border-2 border-gray-200 text-gray-700 rounded-lg hover:bg-gray-50 transition-all duration-300 shadow-sm hover:shadow-md transform hover:-translate-y-0.5 flex items-center gap-2"
      >
